...
async def get_kol_tweets(
    ...
    stock_related: Optional[bool] = Query(None, description="Filter by stock-related posts"),
    ...
    try:
        ...
        if raw_tweets:
            try:
                ...
                twitter_usernames = list(set(
                    row["username"] for row in raw_tweets 
                    if row.get("platform") == "twitter" or not row.get("platform")
                ))
                xhs_user_ids = list(set(
                    row.get("author_platform_id") for row in raw_tweets 
                    if row.get("platform") == "xiaohongshu" and row.get("author_platform_id")
                ))
        ...
        def get_profile_for_row(row):
            ...
            if platform_val == "xiaohongshu":
                return profiles_map.get(("xiaohongshu", row.get("author_platform_id")), {})
            return profiles_map.get(("twitter", row["username"]), {})
        
        tweets = [
            convert_row_to_tweet(row, get_profile_for_row(row))
...
async def get_user_tweets(
    ...
    获取特定用户的推文/帖子
    """
    return await get_kol_tweets(page=page, page_size=page_size, platform=platform, username=username)
