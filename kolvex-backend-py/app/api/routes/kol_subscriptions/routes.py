...
# 统一使用小写平台名称: twitter, reddit, youtube, xiaohongshu
VALID_PLATFORMS = {"twitter", "reddit", "youtube", "xiaohongshu"}
...
def enrich_subscription_with_profile(
    subscription: dict, profile: dict
) -> KOLSubscriptionResponse:
    ...
    return KOLSubscriptionResponse(
        ...
        kol_name=(
            ...
            or subscription["kol_id"]
            if profile
            else subscription["kol_id"]
        ),
        kol_avatar_url=profile.get("avatar_url") if profile else None,
        kol_username=(
            profile.get("username") or subscription["kol_id"]
            if profile
            else subscription["kol_id"]
        ),
...
async def check_kol_tracked(
    ...
    try:
        ...
        return KOLTrackedCheckResponse(
            kol_id=kol_id,
            platform=platform,  # 返回 API 格式的小写平台名
