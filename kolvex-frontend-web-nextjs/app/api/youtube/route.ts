...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter YouTube posts
    let filteredPosts = mockSocialPosts.filter((p) => p.platform === "YOUTUBE");
    ...
    // Transform to YouTube video format
    const videos: YouTubeVideo[] = paginatedPosts.map((post) => {
      ...
      return {
        ...
        channel_avatar_url: creator?.avatar_url || "",
